...
export default function CustomerDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: customer, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await customersApi.getOne(params.id)
      return data
    },
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => customersApi.update(params.id, data),
    onSuccess: async () => {
  ...
  const statusMutation = useMutation({
    mutationFn: (data: { status: 'aktif' | 'nonaktif', keterangan: string }) =>
      customersApi.toggleStatus(params.id, data.status, data.keterangan),
    onSuccess: () => {
  ...
  const { data: statusHistory } = useQuery({
    queryKey: ['customer-status-history', params.id],
    queryFn: async () => {
      const { data } = await customersApi.getStatusHistory(params.id)
      return data
    },
    enabled: !!customer
  })
  ...
  return (
    <AdminLayout>
      ...
      {editFormData && (
        ...
        >
          <form onSubmit={handleUpdateSubmit} className="space-y-5">
            ...
            {/* Tarif */}
            <div>
              <label htmlFor="edit-tarif-id" className="block text-sm font-bold text-gray-700 mb-1.5">
                Kategori Tarif <span className="text-red-500">*</span>
              </label>
              <select
                id="edit-tarif-id"
                value={editFormData.tarif_id}
                onChange={(e) => setEditFormData({ ...editFormData, tarif_id: e.target.value })}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                required
              >
                ...
                {tariffsData?.map((tariff: any) => (
                  <option key={tariff.id} value={tariff.id}>
                    {tariff.nama_kategori} - {formatCurrency(tariff.harga_per_bulan)}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Status */}
              <div>
                <label htmlFor="edit-status" className="block text-sm font-bold text-gray-700 mb-1.5">
                  Status
                </label>
                <select
                  id="edit-status"
              ...
              {/* Tanggal Bergabung */}
              <div>
                <label htmlFor="edit-tanggal-bergabung" className="block text-sm font-bold text-gray-700 mb-1.5">
                  Tanggal Bergabung
                </label>
                <input
      ...
      {/* Tariff Override Modal */}
      <Modal
        isOpen={overrideModalOpen}
        onClose={() => setOverrideModalOpen(false)}
        title="Edit Tarif Bulan Tunggakan"
        size="md"
      >
        <div className="space-y-5">
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-blue-900 font-bold">
                Edit tarif untuk: {formatMonth(selectedMonth)}
              </p>
              <p className="text-xs text-blue-700 mt-1 leading-relaxed">
                Perubahan ini akan langsung memperbarui total tunggakan pelanggan. Pastikan nominal yang dimasukkan sudah benar.
              </p>
            </div>
          </div>
          ...
          <Input
            ...
            placeholder="Contoh: Diskon 50%, Promo khusus, dll"
          />
      ...
      >
        <form onSubmit={handleToggleStatus} className="space-y-4">
          <div className={`p-4 rounded-xl border flex gap-3 ${customer.status === 'aktif'
            ? 'bg-amber-50 border-amber-100 text-amber-800'
            : 'bg-green-50 border-green-100 text-green-800'
            }`}>
            <div className="mt-0.5">
              {customer.status === 'aktif' ? <AlertCircle className="w-5 h-5" /> : <CheckCircle className="w-5 h-5" />}
            </div>
            <div>
              ...
              <p className="text-sm opacity-90">
                {customer.status === 'aktif'
                  ? 'Pelanggan tidak akan ditagih biaya bulanan selama status nonaktif. Tagihan yang sudah ada tetap harus dibayar.'
                  : 'Pelanggan akan kembali ditagih biaya bulanan mulai dari tanggal pengaktifan kembali.'}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder={customer.status === 'aktif' ? "Contoh: Pindah rumah, Cuti sementara, dll" : "Contoh: Kembali berlangganan, dll"}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            />
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button
              ...
              className={customer.status === 'aktif'
                ? 'bg-amber-600 hover:bg-amber-700 text-white border-amber-600'
                : 'bg-green-600 hover:bg-green-700 text-white border-green-600'}
            >
              {customer.status === 'aktif' ? 'Nonaktifkan Sekarang' : 'Aktifkan Sekarang'}
            </Button>
          </div>
        </form>
      </Modal>
