...
export default function EditCustomerPage({ params }: PageProps) {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const [customerRes, tariffsRes, wilayahRes] = await Promise.all([
                ...
                setFormData({
                    ...
                    tanggal_bergabung: customer.tanggal_bergabung ? new Date(customer.tanggal_bergabung).toISOString().split('T')[0] : ''
                })
                ...
                // Optional: redirect back if not found, but maybe let user see empty form or error
            } finally {
                setIsFetching(false)
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.nama || !formData.tarif_id || !formData.alamat || !formData.wilayah) {
